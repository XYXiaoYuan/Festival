...
extension LunarSolarConverter {
    ...
    static func lunarToSolar(lunar: Lunar) -> Solar? {
        ...
        guard let solar11 = LunarSolarConverter.solar_1_1[safe: lunar.lunarYear - LunarSolarConverter.solar_1_1[0]] else {
            return nil
        }
    ...
    static func solarToLunar(solar: Solar) -> Lunar? {
        ...
        var solar11 = 0
        if let solar00 = solar_1_1[safe:index] {
            if solar00 > data {
                index -= 1
            }
        }
        guard let newSolar = solar_1_1[safe: index] else {
            return nil
        }
        solar11 = newSolar
...
extension Array {
    subscript(safe idx: Index) -> Element? {
        if indices ~= idx { return self[idx] }
        return .none
    }
}
