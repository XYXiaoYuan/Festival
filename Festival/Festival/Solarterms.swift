...
private struct SolarTerm {
    ...
    init(solarIndex: Int, solarDate: Int) {
        self.solarIndex = solarIndex
        self.solarDate = solarDate
    }
}

class Solarterms: NSObject {
        
    static func solartermIndex(year: Int, month: Int, day: Int) -> Int {
        ...
        if self.solartermRangeContains(range: ranges[guess], day: day) || self.solartermRangeContains(range: ranges[guess + 1], day: day) {
            var solarTerms: [SolarTerm] = Array(repeatElement(SolarTerm(solarIndex: 0, solarDate: 0), count: 2))
    ...
    static func solartermName(index: Int) -> String {
        if index >= 0 && index < 24 {
    ...
    private static func compressDate(year: Int, month: Int, day: Int) -> Int {
        return (year << 7) | (month << 5) | day
    }
    
    private static func solartermRangeContains(range: Int, day: Int) -> Bool {
        return (day >= (range & 0x1F) && day <= (range >> 5))
    }
    
    private static func ifGregorian(y: Int, m: Int, d: Int, opt: Int) -> Int {
        if opt == 1 {
    ...
    private static func dayDifference(y: Int, m: Int, d: Int) -> Int {
    ...
    private static func equivalentStandardDay(y: Int, m: Int, d: Int) -> Double {
        //Julian的等效标准天数
        let yD = Double(y)
    ...
    private static func term(y: Int, n: Int, pd: Int) -> Double {
        let yD = Double(y)
        let nD = Double(n)
        // 儒略日
    ...
    private static func antiDayDifference(y: Int, x: Double) -> Double {
        var m: Int = 1
        var xD: Double = x
        for j in 1...12 {
